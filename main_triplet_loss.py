...
# %% Get the model
model = lenet_model.get_lenet_model(input_shape=input_shape)
model.summary()
...
x_2 = x_test[y_test == 2]
encoding_2_0 = tf.math.l2_normalize(model(x_2[[0],]))   # this strange [[0],] indexing is to keep dimensions after index slicing
encoding_2_1 = tf.math.l2_normalize(model(x_2[[20],]))

x_5 = x_test[y_test == 5]
encoding_5_0 = tf.math.l2_normalize(model(x_5[[0],]))
encoding_5_1 = tf.math.l2_normalize(model(x_5[[20],]))

x_6 = x_test[y_test == 6]
encoding_6_0 = tf.math.l2_normalize(model(x_6[[0],]))
encoding_6_1 = tf.math.l2_normalize(model(x_6[[20],]))

x_9 = x_test[y_test == 9]
encoding_9_0 = tf.math.l2_normalize(model(x_9[[0],]))
encoding_9_1 = tf.math.l2_normalize(model(x_9[[20],]))
