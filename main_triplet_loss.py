...
threshold_range = np.linspace(0.1, 1.2, num=12)
...
for i, threshold in enumerate(threshold_range):
    ...
    number_positive_pairs = np.sum(really_equal)-len(y_test)
    number_negative_pairs = pairwise_dists.size - number_positive_pairs - len(y_test)

    TP = np.sum(predicted_equal[really_equal]) - len(y_test)
    FN = number_positive_pairs - TP
    FP = np.sum(predicted_equal[~really_equal])
    TN = number_negative_pairs - FP

    recall[i] = TP / number_positive_pairs
    FAR[i] = FP / number_negative_pairs
